...
char *getFingerCapture(PT_DATA io_px_data);
...
char * getFingerCapture(PT_DATA io_px_data)
{
   ...
   if (l_i_Ret==MORPHO_OK)
   {
		...
		char *f_Print = (char *) malloc (sizeof(char)*l_ul_lenTemplate);

		int i=0;
		while (i!=l_ul_lenTemplate)
		{
			f_Print[i]=(char)l_puc_dataTemplate[i];
			printf("dataTemp[%d] : %c res[%d] %c\n",i,l_puc_dataTemplate[i],i,f_Print[i]);
			i++;
		}

		fprintf(stdout,"str : %s size : %d\n",f_Print,sizeof(f_Print));
		...
		{
			fwrite(f_Print,1,l_ul_lenTemplate,l_px_outputfile);
...
char f_Print;
	...
	NAN_METHOD(getFinger)
	{
		...
		//info.GetReturnValue().Set(Nan::New<StringObject>(fingerPrint));
	}
